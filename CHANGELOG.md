...
- **The `hidden` mount mode is now documented as files-only, and the README no longer claims the sandbox exposes "nothing else".** The `devsandbox config init` template and the `MountRule.Mode` schema comment both described `hidden` as hiding "the file/directory", but the builder skips directory matches (logging a warning to the sandbox log) and only overlays files with `/dev/null` - so a rule aimed at a secrets *directory* left it fully readable inside the sandbox, and the template's own example advertised exactly that. Both surfaces now state files-only; `docs/sandboxing.md` and `docs/configuration.md` already did. Separately, the README's isolation summary said the agent sees the project directory and your tools "and nothing else", while the defaults also mount read-only host system paths (`/usr`, `/lib`, `/bin`, a few `/etc` files) and the shell, editor, and sanitized git configs those tools need; it now names those and links the [mount layout](docs/sandboxing.md#filesystem-isolation). What is *not* mounted - SSH keys, cloud credentials, sibling projects, parent directories - is unchanged. Documentation only - no behavior changed.
