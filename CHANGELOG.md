...
- **A misspelled config key is now reported instead of silently doing nothing.** devsandbox ignores keys it does not know, so `keep_containers` instead of `keep_container` left the setting unapplied with no sign anything was wrong. Every config file it loads - the global config, each matching include, and a trusted `.devsandbox.toml` - is now checked at startup, and unknown keys are named on stderr with their full dotted path. Startup is not blocked and the keys stay ignored, so nothing needs to change. `[tools.<name>]` and `[proxy.credentials.<name>]` are never reported: the tool or injector that owns them parses their contents. See [Unrecognized Keys](docs/configuration.md#unrecognized-keys).
