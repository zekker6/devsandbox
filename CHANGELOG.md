...
- krun proxy mode now restricts guest access to the proxy port only; other host-loopback services are no longer reachable via the gateway. `--map-host-loopback` maps the pasta gateway (`10.0.2.2`) to the host's `127.0.0.1` for *every* port, and the egress route surgery kept a `/32` to that gateway on all ports, so the guest could reach any host-loopback service (databases, admin UIs, other daemons) through the gateway, bypassing the proxy filter. pasta has no port-scoped host-loopback option (confirmed against the current `pasta(1)` man page: `--map-host-loopback` takes an address only), so the lockdown now adds a fail-closed firewall in the VMM network namespace - alongside the route surgery, applied via the same `nsenter --user --net` - that allows only new TCP to the gateway on the proxy port (plus established/related return traffic) and drops every other new connection to the gateway. It prefers `nft` and falls back to `iptables`; if neither is available the launch aborts fail-closed rather than run with the port-wide exposure. `devsandbox doctor` now reports a `krun: firewall` row on Linux so a missing backend is caught before launch instead of at it, and the krun prerequisites in the docs list `nft`/`iptables` for proxy mode.
