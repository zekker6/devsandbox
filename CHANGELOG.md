...
- **New `devsandbox agent-wrappers install|status|uninstall`, so supported agents run sandboxed by default.** After `install`, typing `claude` runs `devsandbox claude` in the current directory with every argument passed through, while `claude-no-ds` and `command claude` still reach the real binary. **No startup file is ever edited**: fish gets a `conf.d` drop-in, bash and zsh get a standalone file plus a printed `source` line to add yourself. Nothing is wrapped inside a sandbox. The feature is independent of herdr and useful on its own. See [Tools: Shell wrappers](docs/tools.md#shell-wrappers---run-agents-sandboxed-by-default).
...
- **Agent wrappers say what to do after a devsandbox upgrade instead of failing with command-not-found.** The snippet bakes in devsandbox's absolute path, which the documented `mise use -g` install moves on upgrade. The wrapper now prints `run 'devsandbox agent-wrappers install' to refresh the wrappers` and exits 127. It fails closed on purpose: it neither falls through to the unwrapped agent nor resolves `devsandbox` through `PATH`, which may name a directory sandboxed code can write to.
