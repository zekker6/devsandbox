...
- A setting whose value is of the wrong type now fails the load instead of being ignored. `[tools.docker] enabled = "true"` or `[proxy.credentials.github] overwrite = 1` used to leave the setting at its default with nothing said, so the sandbox ran without something the config plainly asked for. The error names the key and both types; `devsandbox doctor` shows it in the `config` row. Fix the value or drop the key - quoted booleans are the common case. See [Values of the Wrong Type](docs/configuration.md#values-of-the-wrong-type).
...
- Config keys devsandbox does not recognize are now reported on stderr under their full dotted path, so `keep_containers` instead of `keep_container` no longer leaves the setting quietly at its default. Covers the global config, includes, `.devsandbox.toml`, and the `[tools.<name>]` and `[proxy.credentials.<name>]` sections. The keys stay ignored; the launch now stops to have the warning confirmed. See [Unrecognized Keys](docs/configuration.md#unrecognized-keys).

### Changed

- A tool reporting that it started is no longer a warning. The docker, kitty and herdr socket proxies announcing themselves, and herdr finding no host socket to attach to, describe a sandbox behaving as configured - nothing to fix - so they are informational now and no longer count toward the startup warning prompt below. Warnings are for something you have to act on. Messages narrowed by `--no-mitm` stay warnings: they only appear when credential injection, redaction or filtering is configured too, and then report that the control reaches less than it looks.
- A launch that raised warnings now reprints them as one block and asks `Start the sandbox anyway? [y/N]` instead of starting with them unread. `--yes` starts without the prompt, and a launch without a terminal on both stdin and stderr prints the block and continues, so neither a scripted run nor one that redirects stderr can block on a question nobody sees. See [Startup Warnings](docs/sandboxing.md#startup-warnings).
