...
- **An OOM-killed sandbox is no longer silent.** A sandbox that hit its memory limit - or was picked by the host's OOM killer - used to just disappear, taking the agent with it and leaving nothing to explain why. On the bwrap backend devsandbox now watches the sandbox's cgroup for the whole session and reports every OOM kill three ways: a message on stderr as it happens, a `sandbox.oom` audit event, and a record in the sandbox's metadata that `devsandbox sandboxes list` shows in its `STATUS` column as `oom-killed` (the sandbox died) or `oom-kills(N)` (it survived, having lost N processes). The record outlives the killed process and is cleared when the next session on that sandbox starts. Nothing to configure. On bwrap it is active whenever `[sandbox.resources]` limits are set, which is what gives the sandbox a cgroup of its own and so the only state in which a kill is attributable to it rather than to anything else sharing the invoking shell's cgroup; a bwrap sandbox running *without* limits gets a weaker signal instead - if it exits on SIGKILL, devsandbox reports that and says plainly that it cannot tell whether the OOM killer was responsible. On docker and krun the container always has its own cgroup, so it is always active there, including for processes started by `docker exec`. Three cases are not covered: a container launched with `keep_container = false` (anonymous, so the engine cannot be asked for its cgroup), an OOM *inside* a krun guest (decided by the guest kernel, never reaching a host counter), and macOS (the engine's PIDs belong to the VM its daemon runs in). See [OOM Tracking](docs/configuration.md#oom-tracking).

- **A misspelled config key is now reported instead of silently doing nothing.** devsandbox ignores keys it does not know, so `keep_containers` instead of `keep_container` left the setting unapplied with no sign anything was wrong. Every config file it loads - the global config, each matching include, and a trusted `.devsandbox.toml` - is now checked at startup, and unknown keys are named on stderr with their full dotted path. The keys stay ignored, so nothing about the sandbox changes; the launch does now stop to have the warning confirmed (see below). `[tools.<name>]` and `[proxy.credentials.<name>]` are never reported: the tool or injector that owns them parses their contents. See [Unrecognized Keys](docs/configuration.md#unrecognized-keys).

### Changed

- **A launch that produced warnings now stops for confirmation instead of starting anyway.** Warnings were printed as they happened and then buried: sandbox setup is verbose, and the workload takes the terminal a second later, so an ignored config key or a proxy control narrowed by `--no-mitm` went unread. Every warning raised while preparing the sandbox is now collected and reprinted as one block right before the workload starts, followed by `Start the sandbox anyway? [y/N]`. Answering anything but yes aborts with a non-zero status. `--yes` starts without the prompt, and a launch with no terminal on stdin prints the block and continues, so scripted runs never fail on a warning alone. See [Startup Warnings](docs/sandboxing.md#startup-warnings).
- **The `.devsandbox.toml` trust prompt now shows only the settings devsandbox recognizes.** It used to print the file verbatim, which let an untrusted project pad the approval screen with comments and invented keys that read like configuration but do nothing. The prompt is now rendered from the recognized keys alone, so what you approve is what will apply; anything else is dropped from the display and reported as an unknown key. Trust is unchanged in scope - it still covers the whole file by hash, so editing even an ignored key prompts again. A `.devsandbox.toml` that is not valid TOML is now reported as an error rather than offered for approval. See [Local Config Files](docs/configuration.md#local-config-files).
