...
- **Proxy-mode and `.env` masking claims now state their actual limits wherever they appear.** `devsandbox --help`, the `devsandbox config init` template, the README, and `docs/proxy.md`'s bwrap walkthrough each described proxy mode as routing *all* HTTP(S) traffic and `.env` masking as unconditional. Neither holds: bwrap's route surgery is best-effort (the `ip route` calls discard errors and the workload runs regardless, and the interface's own connected subnet survives the default-route deletion), Docker is env-var routing with no network-level enforcement, and only krun fails closed; `.env` masking scans 3 directory levels below the project root and skips `node_modules`, `.git`, `vendor` and `.venv`. These surfaces now carry the qualifier and point at [per-backend behavior](docs/proxy.md#backend-specific-behavior). Documentation only - no behavior changed.
