...
**A wrapper-only agent needs just the `agentid` entry; herdr session capture is what adds the other three.** The `agents` table drives two things on its own: the shell wrappers (`agentid.KnownAgents`) and the run-agent worktree guard (`IsResumeInvocation`). An agent herdr cannot restore - herdr v0.7.4 compiles resume plans for `claude`/`pi`/`codex` only - belongs in the table with an empty `resumeFlag` and its own resume verbs in `resumeAliases` (`opencode`, `copilot` are the worked examples), and it deliberately gets no `ToolWithAgentSessionDir`: wiring capture for a launch herdr will never replay is dead code that also flips such a launch onto the filtered proxy for no benefit. A new row also reserves its name, case-insensitively, in `shell_wrappers.commands` (`shellwrap.ValidateCommandName`), so a config that already lists it fails validation after the upgrade - record the addition under Breaking Changes.

**Adding an agent herdr *can* restore touches four places, and missing one fails silently.** `internal/agentid`'s `agents` table (the name, plus the resume shape herdr compiles in - `claude`/`pi` use a flag, `codex` a `resume` subcommand); the tool registered under that same name; `ToolWithAgentSessionDir` on that tool, derived from the same helper its bindings use; and a `CategoryData` binding for that directory so sessions survive the sandbox. The last two fail in opposite directions - a missing binding discards the session, a bound that drifts from the binding denies every real report. A binding alone is not enough either: `Optional` bindings whose host source is missing are skipped, so a directory the agent only ever creates in-sandbox needs a `ToolWithSetup` that creates it on the host (`Codex.Setup` is the worked example).
...
`internal/sandbox/tools/sharedtmp.go` owns the one directory bind-mounted read-write at an *identical* path on host and sandbox, plus the `$TMPDIR` export that points at it. A tool declares `ToolWithSharedTmp` and gets the mount, the export and the cleanup; it must not emit any of the three itself. Two tools emitting the same mount is a `trackMount` panic, which is why `internal/sandbox/builder.go` and `internal/isolator/docker.go` each emit it once for the first enabled tool that declares it rather than per tool.

- **`$TMPDIR` is global, and that is a cost, not a feature.** It points here only because the launchers that need the directory are third-party shell scripts resolving `${TMPDIR:-/tmp}` with no other hook. The consequence is that every temporary file in the sandbox lands on host-visible disk instead of the tmpfs `/tmp` the builder mounts - Go build temporaries, test scratch trees, agent scratchpads. That reached 500 MB in one project before anything reclaimed it. Do not widen the blast radius further by moving more state here.
- **Cleanup is gated on liveness, not on age alone.** A cold start (no other devsandbox process registered under this sandbox home's `.run/`) empties the directory; a live sibling downgrades that to removing entries with nothing modified for 7 days. Wiping unconditionally is the bug this replaced: it yanked state from under a running tenant, and a non-recursive `mkdir` then failed with ENOENT.
- **Register before observing.** `prepareSharedTmp` calls `ensureRunDir` *first*, then looks for siblings. Two sessions starting together then each see the other and both take the conservative path; the reverse order lets both conclude they are alone and delete each other's files.
- **Age must be judged over the whole subtree.** A directory's mtime moves only when a direct child changes, so a tenant writing deep inside a tree it created days ago looks stale to a shallow `Stat`. `modifiedSince` walks, and treats any unreadable entry as recent - an unknown age must never authorize deletion.
- **The directory is keyed on a hash of the sandbox home, so there is exactly one spelling of that path.** `sandbox.SandboxHomePath(root)` is it; a caller that spells it any other way - through a symlink, with a trailing slash, from a staged copy under `.removing/` - hashes to a name nothing ever created and removes nothing, with no error. `reclaim.liveSandboxHomes` builds its live set through that same function rather than joining `"home"` itself, or the orphan sweep would name every live sandbox an orphan.
- **Keying on the path and not the inode is also why the *rename* runs under the lock.** Staging the sandbox tree aside does not take the shared temp's name away from anyone: `acquireSharedLock` reads a missing root as "the sandbox is gone" and recreates it at the same path, which hashes to the same directory. `stageForRemoval` therefore renames it aside (`tools.StageSharedTmpForRemoval`) while still holding the exclusive lock, where the sandbox is provably idle - a removal running after the lock was released deletes the `$TMPDIR` of the launch that just took its place. Only the rename: the delete is a chmod walk over everything the sandbox put in `$TMPDIR`, and `TeardownGracePeriod` is already spent in full by `beforeRemove`, so unbounded work added under the lock aborts the launch `sharedLockRetries` is sized to hold on. The staged name is not a session hash, so a kill between the two leaves an orphan the 7-day sweep reclaims.
- **A configured base is not the full ownership list.** `sharedtmp_owner.go` records each sandbox home under host-owned state before creating its temporary directory. Prune consults those records across base changes and preserves unknown hashes; absence from the current base plus age never proves an orphan. An inaccessible recorded base also blocks deletion, since it may be an unmounted volume.
- **Nothing the host trusts may live here.** The sandbox can write every byte of it. Host-owned records go under `$XDG_STATE_HOME/devsandbox/`; see *State the host trusts* above.

## Host-owned state and the reclaim catalogue

`internal/reclaim` catalogues every place devsandbox writes state on the host for its own bookkeeping - twelve `Location`s, each naming itself, its path and the function that sweeps it. It is a list of *places*, not of moments: every component keeps its sweep where it is and the launch keeps calling it in the order the launch needs, and the catalogue only points at that logic. That is what gives `sandboxes prune` one command that sweeps every location, one report shape for what a sweep did, and a reviewer one list to check a new state directory against. A `BeforeRun`/`AfterRun` hook bus was considered and rejected: `AfterRun` is `defer` with a registry in front, and every leak in these locations exists precisely because the process was killed before its own cleanup could run.

- **State devsandbox keeps for itself must record an owner per entry and appear in the catalogue.** The owner is a pid in the entry name, a held lock, or a reference to state elsewhere whose absence proves the entry abandoned - herdr pane records name the sandbox root their launch used, and go when it does. `TestLocations_CatalogueIsComplete` pins the twelve names in order, so a location added without registering it, or one silently retired, fails there. Reclaiming by **age alone** is the weaker rule, because it cannot tell an abandoned entry from an idle one; where it is the only rule available it has to be justified at the registration, as the orphaned shared temp's 7 days is.
- **The pid probe answers uncertainty as alive, so a pid-keyed location needs an age backstop or an accepted leak.** `procstate.Alive` answers dead only when the kernel says the process is gone; EPERM - a pid recycled by *another user's* process - reads as alive, so an uncertain probe never authorizes a deletion. Every one of these directories is per-user, so that answer is conservative rather than correct and the entry would otherwise survive forever. Session records and interrupted removals carry a 30-day backstop, because the first holds its session name against every later launch and the second is an entire sandbox tree. **A backstop belongs on the uncertain answer and not on "alive", which is why those two read `procstate.Probe` rather than `Alive`:** age says nothing about whether a session is still running - a record is rewritten only when its forwarded ports change - so aging out a pid the kernel confirms is *live* drops a running session from `devsandbox sessions` and frees its name for the next launch, and does the same to a staged tree the teardown holding that pid is still deleting. Egress markers and run directories deliberately carry none: the leak is an empty directory, and a wrong deletion costs a live session its exit-78 marker or its open sockets. Say which of the two it is where the location is registered.
- **`~/.local/share/devsandbox-scratchpads/` is the deliberate exception.** It holds the user's work rather than devsandbox's bookkeeping, its lifecycle is user-directed through `devsandbox scratchpad rm`, and it must never be swept. It is named here as an exception so it does not read as a location someone forgot to register.
- **A second entry point needs its own safety, not the launch's.** A launch establishes primary designation and run-dir registration before it sweeps; `sandboxes prune` establishes neither, so a nil `Sweep` is a real answer rather than an omission. Run directories and session overlay dirs are reported and never swept from prune - the launch already sweeps both, and taking the exclusive lock across a multi-gigabyte overlay delete can exhaust a concurrent launch's retry budget for zero bytes gained - and the live shared temp takes only its age branch, since a prune process is not registered in `.run/` and so cannot conclude it is alone. All three stay in the list, because a list that omits what it does not sweep is no longer the list to check against.
- **The owning packages must never import `internal/reclaim`.** Every `Sweep` is a plain function over plain arguments, so no owner needs the `Location` type, and that is what keeps the catalogue free of cycles. The check is that nothing outside `cmd/` imports it. If an owner ever does need `Location`, split the catalogue into `internal/reclaim/catalog` rather than reaching back.
- **A log the host writes must not live on a path the sandbox can write.** The sandbox home is bound read-write at the host home path, so `<sandboxHome>/logs/internal/` is `$HOME/logs/internal/` inside the sandbox, and a symlink planted there is followed by the host's `O_APPEND` open. Host-written logs belong under `<sandboxRoot>/logs/`, which is not bound - `sandbox.log`, `tools-errors.log`, `<engine>.log` and `logging-errors.log` all live there, and the wrapper log under `$XDG_STATE_HOME/devsandbox/`. `doctor` still reads the old home-side directory as well, deliberately, so sandboxes created before the move keep their logs findable.
- **An append-only host log is bounded by `internal/logrotate`, at the moment it is opened.** Rotating on open is what makes the catalogue entry for the internal error logs reported-only: they are per-sandbox, so that sandbox's next launch bounds them, and a sweep would rotate a file no process holds open and reclaim nothing. The wrapper log rotates on open too - `notice.Setup` does it before the append handle opens - and is still swept, because `notice.Setup` runs only from a launch: a log that grew past the limit during the last launch would otherwise stay over it until the next one, on a host that may not have another.

## Shell snippets evaluated on the host

`internal/shellwrap` generates the snippet `devsandbox shell-wrappers activate <shell>` (alias `agent-wrappers`) prints. devsandbox writes nothing to the user's shell config: the user pastes an activation line that evaluates the command's stdout at every shell start, like `mise activate`. Five things are load-bearing there:
...
- **`activate` must not fail on a host with no agent installed.** It runs on every shell start, so it emits a comment - still valid shell - rather than an error that would break the startup file. The comment is followed by the snippet even when nothing is wrapped: its cleanup is what removes an earlier activation's wrappers, so a comment alone would leave them defined.
- **A failure has to distinguish host state from a bad activation line.** The bullet above is not a rule that `activate` never fails. An agent that is merely not installed - including one named in `--agents` - degrades to a comment, because it is a fact about this host that installing it later fixes on its own. A `--agents` value naming an unsupported agent, or an explicitly empty one, is wrong at *every* shell start until the line is edited, so it exits non-zero naming the supported agents. Both are settled before any discovery runs and before a byte reaches stdout (`resolveAgentSelection` ahead of `resolveWrapperEnv` in `cmd/devsandbox/agentwrappers.go`), or a rejected selection would leave a startup file evaluating half a snippet. The comment itself may call the list "supported" only when it *is* `agentid.KnownAgents()`; a narrowed selection worded that way asserts devsandbox supports nothing else. `agentid.KnownAgents` order is a documented contract now - `docs/tools.md` spells the emission order out - so reordering that table makes the docs wrong.
- **A project `.devsandbox.toml` never decides whether a shell gets wrappers.** `activate` loads config the way a launch does, and the loader reads and parses that file before asking for trust - while the project directory is writable from inside the sandbox. So `configuredCommands` retries a failed load without the project file and warns on stderr; only a failing global config or include is returned as an error, because the sandbox cannot write those. The retry is not enough on its own: `LoadConfigWithOptions` calls `os.Getwd()` even with `SkipLocalConfig`, and the sandbox can remove the project subdirectory a host shell sits in, so an unresolvable directory falls back to `config.LoadFrom` on the global config. Returning the project error meant sandboxed code could plant an unparsable file and leave the next host shell with no `claude` wrapper, so herdr's typed resume ran on the host. For the same reason activation never prompts for trust (`declineProjectTrust`): the prompt reads stdin at shell start, where herdr has typed `claude --resume <id>` into the pane, so a sandbox that changed the file's hash had the prompt swallow the resume as its answer.
