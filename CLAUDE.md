...
`internal/shellwrap` generates the snippet `devsandbox agent-wrappers activate <shell>` prints. devsandbox writes nothing to the user's shell config: the user pastes an activation line that evaluates the command's stdout at every shell start, like `mise activate`. Four things are load-bearing there:
