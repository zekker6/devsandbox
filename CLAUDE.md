...
- **An append-only host log is bounded by `internal/logrotate`, at the moment it is opened.** Rotating on open is what makes the catalogue entry for the internal error logs reported-only: the next launch bounds them, so a sweep would rotate a file no process holds open and reclaim nothing. The wrapper log is the exception that is swept, because it has no per-launch open of its own to piggyback on.
