...
// TestDockerBackend_HideEnvFiles tests that .env files are hidden
// inside the Docker container.
// NOTE: This feature requires mount --bind inside the container which needs
// elevated privileges (--privileged or CAP_SYS_ADMIN). In regular Docker mode,
// the hiding may not work, so this test is skipped if hiding fails.
func TestDockerBackend_HideEnvFiles(t *testing.T) {
	if testing.Short() {
		t.Skip("Skipping e2e test in short mode")
	}
	...
	// Create a temp config directory with docker isolation and hide_env_files enabled
