...
**This directory also bounds the completion sentinel.** A launch command carries the sentinel path as text, and the clause the launcher appends is not a `touch` but `printf "%s" "$rc" > '<sentinel>'.tmp && mv -f '<sentinel>'.tmp '<sentinel>'` - so a path that was only checked for *shape* let sandboxed code have the host truncate and overwrite any file the invoking user can write. The path must now sit inside this directory, and the bound is taken from the same function that produces the mount, so the two cannot drift apart. A launch for which devsandbox cannot derive the directory is denied rather than accepted unbounded. The file a herdr launch script captures the viewer's stderr into is bounded the same way, for the same reason.
...
The **option words are an allowlist keyed on the editor**, not a shape. Editors that detach have to be told to block, so `EDITOR="code --wait"`, `subl -w` and `emacsclient -t` all work - but every editor in the list also has at least one flag that makes its trailing operand a *program* rather than a document: `nvim -u` sources the file as the init vimrc, `vim -S` as a Vim script, `emacs -l` as Elisp, `hx -c` as a config, `kak -e` as a command to run. Since the operand is the file in the project tree, accepting any dash-prefixed word would have reinstated exactly the host execution the editor list exists to deny. The same word means different things to different programs - `-c` is `--create-frame` to emacsclient and an ex command to vim - so it is accepted only for the editors that treat it as a switch. A value that matches your host's own `EDITOR`/`VISUAL` byte for byte is taken whole, flags included: that is the setting the host would have run anyway. Only the flags ride on that match - the program word goes through the `PATH` rule below like any other, since "the setting the host would have run" still names a file by name, and which file that is comes from the same lookup.

Relative paths, values carrying a path argument or shell-significant characters, flags outside that editor's list, and names outside the table are refused rather than interpreted. The `env` program itself is pinned to `/usr/bin/env` or the host's own resolution of `env`, never to a path that merely ends in `/env`.

The host's `PATH` lookup is the anchor for every form - bare name, absolute path, and your own host setting alike - so it is checked in turn: if that lookup lands **inside the project directory or the [shared temp directory](#shared-temp-directory)**, the value is refused whichever way it was written. Both are bind-mounted read-write, so a program found there is one the sandbox wrote - and a project-local bin directory on `PATH` is ordinary (`.venv/bin`, `node_modules/.bin`, a `bin` direnv or mise adds). The same rule applies to `env`, and to the `sh` the kitty launch patterns accept: if the host resolves `sh` inside one of those directories, revdiff's kitty launch patterns are dropped entirely and every launch request is denied, since the argv reaches kitty as the sandbox wrote it.

Where a name is spelled bare, **both halves of the lookup are checked** - the directory entry `PATH` found and the file it points at. A `bin/sh` symlink in your project aimed at the real `/bin/sh` resolves outside every bound, but the file the terminal opens is the link, and the sandbox can repoint it after the check and before the launch. The `revdiff` binary the patterns pin is bounded the same way: an installation the host resolves inside the project directory or the shared temp directory drops the patterns, because the pin would otherwise name a file whose contents the sandbox chooses. A revdiff installed anywhere else - the usual case - is unaffected.

**Both bounds cover both spellings of their directory.** devsandbox learns the project directory as your shell reports it, which keeps a symlink you entered it through, while the mount follows that link to the tree behind it. A project entered as `~/proj-link` is therefore refused for a program `PATH` resolves under `~/proj-link/.venv/bin` *and* for one it resolves under the real path that link points at - both spellings reach the same read-write mount.
...
| `--source-window`, `--next-to`, `--add-to-session`, `--os-panel` | Denied. |
| `--hold`, `--hold-after-ssh` | Denied - kitty runs the host user's shell in the window after the launched command exits, which would put a program the allowlist never approved behind an approved launch. |
| `--color` | Only the inline `name=value` form. A spec without `=` is a path kitty opens and parses, so it reads a host file the sandbox names. |
...
| Titles, placement, `--keep-focus` | Allowed - none of them changes what runs or what it can read. |
...
The copy carries one addition devsandbox makes itself: a `set -C` line after the shebang. The accepted script ends by writing the launch's exit code into `<sentinel>.tmp` and renaming that over the sentinel, and the sentinel lives in the [shared temp directory](#shared-temp-directory) - which the sandbox can write at the same path the host sees. Without `set -C` a symlink planted at `<sentinel>.tmp` is followed by the host's own shell, so that redirect would truncate whatever the link names and write the exit code into it. Under `set -C` the redirect refuses an existing path instead of following it, and the rename that follows acts on the link rather than its target. The command is also handed to `/bin/sh` by absolute path rather than to whatever `sh` the pane's `PATH` resolves.

The script may also redirect the launched program's stderr into a file (`2>'<path>'`, the last thing on the command line and the only redirect accepted): revdiff 0.8.23 and later capture it there because the overlay closes the moment a fast-failing viewer exits, taking the error text with it. That file is a second path the host opens for writing on sandboxed code's word, so it is confined to the shared temp directory the same way the sentinel is, and `set -C` covers it the same way. One extra step goes with it - the launcher creates the file with `mktemp`, and noclobber refuses to open a path that already exists, so devsandbox unlinks it before the run and lets the redirect create it fresh. That unlink cannot be steered outside the shared directory by a symlinked component in the middle of the path, which a purely textual bound would not catch; such a launch is denied instead.
...
"Host-only" is checked, not assumed. The relocation directory is refused if it falls inside anything the sandbox can write at the same path - the sandbox home, the [shared temp directory](#shared-temp-directory), or the project directory, which is bind-mounted read-write at the path the host knows it by. A project directory that contains the relocation root (running devsandbox from `~/.cache`, say) therefore starts the session with a warning and **no** relocator, and every script launch is denied for that session; the rest of the herdr capabilities are unaffected. The comparison takes each path under both its literal spelling and the one it resolves to, so a project entered through a symlink is caught as well - devsandbox is told the link's name while the mount binds the tree behind it, and only one of those two names would otherwise ever be compared.
